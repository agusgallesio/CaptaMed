  ...
  return (
    <div className="card flex h-full min-h-[480px] flex-col">
      <div className="flex items-center gap-2 border-b border-slate-200 px-4 py-3">
        ...
        <div>
          <h2 className="text-sm font-semibold text-ink-900">Asistente CaptaMed</h2>
          <p className="text-[11px] text-slate-500">Preguntale a tus datos de campañas y CRM</p>
        </div>
      </div>
          ...
          <div className="space-y-2 pt-2">
            ...
            {SUGERENCIAS.map((s) => (
              <button
                key={s}
                onClick={() => enviar(s)}
                className="block w-full rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-left text-xs text-slate-700 transition hover:border-brand-300 hover:bg-brand-50"
              >
                {s}
              </button>
            ))}
            ...
            <div
              className={
                m.role === "user"
                  ...
                  : "max-w-[92%] whitespace-pre-wrap rounded-2xl rounded-bl-sm bg-slate-100 px-3 py-2 text-sm text-slate-800"
              }
            >
      ...
      >
        <input
          ...
          className="flex-1 rounded-lg border border-slate-300 px-3 py-2 text-sm outline-none transition focus:border-brand-500 focus:ring-2 focus:ring-brand-100"
          disabled={loading}
        />
        <button
          type="submit"
