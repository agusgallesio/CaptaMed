...
export default function Embudo() {
  ...
  if (loading || !data)
    return <div className="flex h-64 items-center justify-center text-sm text-slate-500">Armando el embudo…</div>;
    ...
    <div className="space-y-4">
      ...
      <section className="card p-4">
        <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
          <h3 className="text-sm font-semibold capitalize text-ink-900">Proyección de {p.mes}</h3>
            ...
            <tbody>
              {ETAPAS_PROYECCION.map((e) => (
                <tr key={e.key} className="border-b border-slate-100 last:border-0">
                  <td className="py-2.5 pr-3 font-medium text-ink-800">{e.label}</td>
        ...
        <div className="mt-4 grid grid-cols-2 gap-3 md:grid-cols-2">
          <div className="rounded-lg border border-slate-200 bg-slate-50 p-3">
            <p className="text-[11px] uppercase tracking-wide text-slate-500">Inversión proyectada</p>
            <p className="mt-0.5 text-lg font-semibold tabular-nums text-ink-900">
              ≈ {fmtMoney(p.proyectado.inversion)}
            </p>
            <p className="text-xs text-slate-500">Acumulado: {fmtMoney(p.actual.inversion)}</p>
          </div>
          <div className="rounded-lg border border-brand-200 bg-brand-50 p-3">
            <p className="text-[11px] uppercase tracking-wide text-brand-700">Venta proyectada</p>
            <p className="mt-0.5 text-lg font-semibold tabular-nums text-brand-800">
              ≈ {fmtMoney(p.proyectado.ventaTotal)}
            </p>
            <p className="text-xs text-brand-700/70">Acumulado: {fmtMoney(p.actual.ventaTotal)}</p>
          </div>
        </div>
